...
Errors:
 ...
 NameTooLsource/symmetry/api/libzfs_core.d.tmp:6490:67: warning: missing terminating ' character
     NameTooLong: if the dataset name is too longor  if the dataset's origin has a snapshot that, if transferred to the dataset, would get a too long name.
                                                                   ^
source/symmetry/api/libzfs_core.d.tmp:6493:98: warning: missing terminating ' character
     SnapshotExists: if the dataset already has a snapshot with the same name as one of the origin's snapshots.
                                                                                                  ^
source/symmetry/api/libzfs_core.d.tmp:6513:38: warning: missing terminating ' character
     FilesystemNotFound: if the target's parent does not exist.
                                      ^
ong: if the name or the origin name is too long.
...
`)
auto sendResume(string snapshotName, Socket socket, string fromSnapshot="", SendFlag[] flags=[], ulong resumeObj=0UL, ulong resumeOff=0UL)
{
 import std.algorithm:fold;
 auto lzcFlags = flags.fold!((a,b) => a| b)(cast(SendFlag)0).to!lzc_send_flags;
 auto result = lzc_send_resume(snapshotName.toCString, fromSnapshot.toCString, cast(int)socket.handle, lzcFlags, resumeObj, resumeOff);
 enforce(result == 0, "zfs error");
}
...
`)
void receive(string snapName, Socket socket, string origin="", bool force=false, bool raw=false, string[string] properties = (string[string]).init, bool resumable = false)
{
 import std.format:format;
 auto props = getProperties(properties);
 scope(exit)
  nvlistFree(props);
 auto result = resumable ? lzc_receive(snapName.toCString, props,origin.toCString, force? 1:0, raw?1:0, cast(int)socket.handle) :
   lzc_receive_resumable(snapName.toCString, props,origin.toCString, force?1:0, raw? 1:0, cast(int)socket.handle);
...
`)
void receiveWithHeader(string snapName, Socket socket, string origin="", bool force=false, bool raw=false, string[string] properties=string[string].init, bool resumable=true, dmu_replay_record* beginRecord=null)
{
 auto props = getProperties(properties);
 scope(exit)
  nvlistFree(props);
 auto result = lzc_receive_with_header(snapName.toCString, props, origin.toCString, force ? 1 :0, resumable ? 1 : 0, raw ? 1 :0, cast(int)socket.handle, beginRecord);
 enforce(result == 0);
}
...
`)
auto receiveOne(string snapName, Socket socket, string origin="", bool force=false, bool resumable=true, bool raw=false, string[string] properties=string[string].init, dmu_replay_record* beginRecord=null, int cleanupFileDescriptor=-1, ulong actionHandle = 0UL)
{
 ulong readBytes;
 ulong errorFlags;
 nvlist_t* errorList;
 ...
 scope(exit)
  nvlistFree(props);
 auto result = lzc_receive_one(snapName.toCString, props, origin.toCString, force ? 1 : 0, resumable ? 1 :0, raw ? 1 : 0, cast(int)socket.handle, beginRecord, cleanupFileDescriptor, &readBytes, &errorFlags, &actionHandle,&errorList);
 enforce(result == 0);
...
this nvlist
`)
auto receiveWithCommandProperties(string snapName, string[string] properties, string[string] commandProperties, ubyte[] keyData, string origin, bool force, bool resumable, bool raw, int inputFileDescriptor, dmu_replay_record* beginRecord, int cleanupFileDescriptor,ulong actionHandle = 0UL)
{
 import std.format:format;
 ulong readBytes;
 ulong errorFlags;
...
string zfsVersion()
{
 ...
 import std.string: splitLines, startsWith, strip, join,split;
 ...
 return lines.front.strip.split.back;
}
...
`)
void inheritProperties(string name, string property, bool recursive=false, bool useSudo = true)
{
 import std.algorithm:each,map;
 import std.format:format;
 import std.string:join,splitLines,split;
 import std.array:Appender;
 Appender!string props;
 import std.format:format;
 import std.process: executeShell;

 DatasetPropertiesResult ret;
 auto commandString = format!"%s inherit %s %s %s"(useSudo ? "sudo ":"", recursive ? "-r ":"", property,name);
 auto result = executeShell(commandString);
 enforce(result.status==0, result.output);
}
...
void setDatasetProperties(string name, string[string] properties, bool useSudo = true)
{
 import std.format:format;
 import std.string:join;
 import std.array:Appender;
 import std.process: executeShell;
 Appender!string props;
 foreach(prop;properties.byKeyValue)
 {
  props.put(prop.key);
  props.put("=");
 ...
 auto commandString = format!"%s zfs set %s %s"(useSudo ? "sudo ":"", props.data, name);
 auto result = executeShell(commandString);
 enforce(result.status==0, result.output);
}
...
struct DatasetPropertiesResult
{
...
DatasetPropertiesResult[string] getDatasetProperties(string name,bool recursive = true, bool useSudo = true)
{
 import std.algorithm:each,map;
 import std.format:format;
 import std.string:join,splitLines,split,replace;
 import std.array:Appender;
 Appender!string props;
 import std.format:format;
 import std.process: executeShell;
 ...
 DatasetPropertiesResult[string] ret;
 auto commandString = format!"%s zfs get all %s %s"(useSudo ? "sudo ":"", recursive ? "-r ":"", name);
 auto result = executeShell(commandString);
 enforce(result.status==0, result.output);
 auto lines = result.output.splitLines;
 auto colNums = lines.front.getColumns();
 auto results = lines.dropOne.map!(line => line.makeColumns(colNums));
 foreach(entry;results)
 {
  if(entry.length>2)
  {
   auto r = ret.get(entry[0],DatasetPropertiesResult.init);
   r.values[entry[1]] = entry[2];
   if(entry.length >3)
    r.sources[entry[1]] = entry[3];
   ret[entry[0]] = r;
  }
 }
 return ret;
...
string[] listChildren(string name,bool useSudo = true)
{
 import std.format:format;
 import std.algorithm:each,map;
 import std.string:join,splitLines,split;
 ...
 auto commandString = format!"%s zfs list -r %s"(useSudo ? "sudo ":"", name);
 auto result = executeShell(commandString);
 enforce(result.status==0, result.output);
 return result.output
  .splitLines
  .dropOne
  .map!(line => line.split(' ').front)
  .array;
}
...
`)
string[] listSnapshots(string datasetName, bool recursive = false, bool useSudo = true)
{
 import std.format:format;
 import std.algorithm:each,map;
 import std.string:join,splitLines,split;
 ...
 auto commandString = format!"%s zfs list %s -t snapshot %s"(useSudo ? "sudo ":"", recursive ? "-r ":"",datasetName);
 auto result = executeShell(commandString);
 enforce(result.status==0, result.output);
 return result.output
  .splitLines
...
int[] getColumns(string header)
{
 ...
 {
  bool isWhiteSpace = header[i].isWhite;
  if (!isWhiteSpace && newColumn)
  {
   ret.put(i++);
   newColumn = false;
  }
...
string[] makeColumns(string line, int[] columns)
{
 import std.array:Appender;
 import std.algorithm:min;
 import std.string:strip;
 Appender!(string[]) ret;
 foreach(i;1..columns.length)
 {
  auto start = columns[i-1];
  auto end = columns[i];
  if (start > line.length)
   break;
  end = min(line.length,end);
  ret.put(line[start..end].strip);
 }
 return ret.data;
}
